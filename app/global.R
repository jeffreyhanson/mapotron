...
makeDirs <- function(dname) {
  dir.create(file.path(data.params.LST[['data.directory']],dname,"temp"),
             showWarnings=FALSE, recursive=TRUE)
  dir.create(file.path(data.params.LST[['data.directory']],dname,"zip"),
             showWarnings=FALSE, recursive=TRUE)
  dir.create(file.path(data.params.LST[['data.directory']],dname,"data"),
             showWarnings=FALSE, recursive=TRUE)
  dir.create(file.path(data.params.LST[['data.directory']],dname,"basemap"),
             showWarnings=FALSE, recursive=TRUE)
}
...
to.geojson.from.SpatialPoints <- function(x, cols, notes, style) {
  list(
    ...
    features=lapply(seq_len(nrow(x@coords)), function(l) {
      list(
        type="Feature",
        properties=list(note=notes[l], color=cols[l], fillColor=cols[l],
    ...
    features=lapply(seq_along(x@lines), function(l) {
      list(
        ...
        geometry=list(
          ...
          coordinates=list(lapply(x@lines[[l]]@Lines,
                                  function(m) {to_list(m@coords)}))
        )
      )
    })
  )
  ...
  list(
    ...
    features=lapply(seq_along(x@polygons), function(l) {
      list(
        ...
        geometry=list(
          type = "MultiPolygon",
          coordinates=list(lapply(x@polygons[[l]]@Polygons,
                                  function(m) {to_list(m@coords)}))
        )
      )
    })
...
to.SpatialPoints.from.geojson <- function(jsonlst, crs=CRS("+proj=longlat +ellps=WGS84 +datum=WGS84 +no_defs ")) {
  ...
  return(SpatialPoints(coords,proj4string=crs))
}
...
to.SpatialLines.from.geojson=function(jsonlst, id, crs=CRS("+proj=longlat +ellps=WGS84 +datum=WGS84 +no_defs ")) {
  ...
  return(SpatialLines(mainLST,crs))
}
...
to.SpatialPolygons.from.geojson <- function(jsonlst, id, crs=CRS("+proj=longlat +ellps=WGS84 +datum=WGS84 +no_defs ")) {
  ...
  return(SpatialPolygons(mainLST,proj4string=crs))
}

to.SpatialPolygons.from.circle <- function(jsonlst, id, radii, crs=CRS('+proj=longlat +ellps=WGS84 +datum=WGS84 +no_defs ')) {
  x=spTransform(gBuffer(spTransform(to.SpatialPoints.from.geojson(jsonlst, crs=crs),
                                    CRSobj=CRS('+proj=merc +a=6378137 +b=6378137 +lat_ts=0.0 +lon_0=0.0 +x_0=0.0 +y_0=0 +k=1.0 +units=m +nadgrids=@null +wktext  +no_defs')),
                        width=radii, byid=TRUE),
                CRSobj=crs)
...
rbind.SpatialLinesDataFrame <- function (..., fix.duplicated.IDs = TRUE) {
    ...
    sp::SpatialLinesDataFrame(do.call("rbind", lapply(dots, function(x) as(x, "SpatialLines"))),
                              do.call("rbind", lapply(dots, function(x) x@data)))
}
