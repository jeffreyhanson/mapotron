...
shinyServer(function(input, output, session) {
  ...
  if (!inherits(email.params.LST, 'try-error')) {
    toc$email <- list(api.key=email.params.LST$api.key,
                      api.url=email.params.LST$api.url,
                      api.address=email.params.LST$api.address)
  } else {
  ...
  session$onFlushed(once=TRUE, function() {
    ...
    if (!is.null(toc$args$emailaddress)) {
      if (file.exists(file.path(data.params.LST[['data.directory']],dname,"basemap"))) {
        files <- dir(file.path(data.params.LST[['data.directory']],dname,"basemap"),
                    '^.*\\.rds', full.names=TRUE)
        for (f in files) {
          d <- readRDS(d)
          if (!is.list(d)) {
          ...
          for (di in seq_along(d)) {
            if (is.Spatial(d[[di]])) {
              if (nrow(baselayers[[i]]@data)<30) {
                ...
                toc$newFeature(paste0('r_',id$new()), d[[di]], 'r',
                               names(d)[di], d[[di]]@data[[1]], currCols)
              }
            }
        }
      }
    }
    # parse arguments to automatically send data on close
    if (!is.null(toc$args$firstname) & !is.null(toc$args$lastname) &
        !is.null(toc$args$emailaddress))
    {
      ...
      addTooltip(session,'emailBtn', 'Data will automatically be emailed.',
  ...
  ## observer for creation of new features
  observe({
    if (is.null(input$map_create))
      return()
    isolate({
      if (is.null(input$map_create$radii)) {
        toc$newFeature(input$map_create$id,
                       RJSONIO::fromJSON(input$map_create$geojson)$geometry,
                       "rw")
      } else {
        toc$newFeature(input$map_create$id, 
                       to.SpatialPolygons.from.circle(RJSONIO::fromJSON(input$map_create$geojson)$geometry, 
                                                      id=input$map_create$id, radii=input$map_create$radii),
                      "rw")
      }
  ...
  ## observer for updating existing features
  observe({
    ...
    isolate({
      for (i in seq_along(input$map_edit$list)) {
        if (is.null(input$map_edit$list[[i]]$radii)) {
          toc$updateFeature(input$map_edit$list[[i]]$id,
                            json=RJSONIO::fromJSON(input$map_edit$list[[i]]$geojson)$geometry)
        } else {
          toc$updateFeature(input$map_edit$list[[i]]$id,
                            sp=to.SpatialPolygons.from.circle(RJSONIO::fromJSON(input$map_edit$list[[i]]$geojson)$geometry,
                            id=input$map_edit$list[[i]]$id,
                            radii=input$map_edit$list[[i]]$radii))
        }
      }
      session$sendCustomMessage("update_var",list(var="is_dirty", val="true"))
    })
  ...
  ## observer for adding notes to existing features
  observe({
    if (is.null(input$map_note))
      return()
    isolate({
      toc$updateFeature(as.character(input$map_note$id),
                        note=sanitise(input$map_note$text))
      map$removePopup("map_add_note")
      session$sendCustomMessage("update_var",list(var="is_dirty", val="true"))
    })
  })

  ## observer for deleting existing features
  observe({
  ...
  ## observer for click event on download button
  observe({
  ...
  ## observer for click event on email button
  observe({
    ...
    isolate({
      ...
      if (!is.null(toc$args$firstname) & !is.null(toc$args$lastname) & !is.null(toc$args$emailaddress)) {
        ...
        if ((!toc$args$emailaddress %in% emailBlockList) & length(toc$features)>0) {
          try(toc$export(toc$args$firstname, toc$args$lastname, 
              toc$args$emailaddress, toc$args$message))
        }
        # post
  ...
  observe({
    ...
    isolate({
      ...
      toc$garbageCleaner()
      # main
      x=list("First Name"=input$firstName, "Last Name"=input$lastName,
